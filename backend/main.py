...
app.add_middleware(
    CORSMiddleware,
    # Wide open for local dev - restrict this before shipping to production.
    allow_origins=["*"],
...
class WorkerAvailabilityUpdate(BaseModel): worker_id: str; is_available: bool
class WorkerRadiusUpdate(BaseModel): worker_id: str; service_radius_km: float
class WorkerProfileUpdate(BaseModel):
    worker_id: str; user_id: str
    name: str; phone: str
    skill_category: str; hourly_rate: float
...
async def create_booking_request(data: BookingRequestModel):
    ...
    if not service_res.data:
        raise HTTPException(status_code=400, detail=f"Category '{data.service_id}' not found in services table.")
        
    real_service_uuid = service_res.data[0]["id"]
    
    group_id = str(uuid.uuid4())
    bookings_to_insert = [{
        "group_id": group_id, "customer_id": data.customer_id, "customer_lat": data.customer_lat,
        "customer_lng": data.customer_lng, "worker_id": w_id, 
        "service_id": real_service_uuid, # Insert the valid UUID here
...
def get_worker_requests(worker_id: str, worker_lat: float, worker_lng: float):
    worker_res = supabase.table("workers").select("skill_category, is_available, service_radius_km").eq("id", worker_id).maybe_single().execute()
    if not worker_res.data:
        raise HTTPException(status_code=404, detail="Worker not found.")
    worker_row = worker_res.data
    worker_skill = worker_row.get("skill_category") or ""
    service_radius_km = float(worker_row.get("service_radius_km") or 15)

    if not worker_row.get("is_available"):
        return {"status": "success", "requests": [], "worker_skill": worker_skill, "service_radius_km": service_radius_km, "count": 0}
    ...
    for req in response.data:
        ...
        requests_data.append({
            "booking_id": req["id"], "group_id": req["group_id"], "customer_id": req.get("customer_id"),
            "customer_name": _customer_name(req), 
            "service_id": worker_skill, # Mask the UUID with the readable string
            "price": req["price"], "distance_km": round(dist_km, 2), "eta_mins": calculate_eta_mins(dist_km),
            "expires_at": req["expires_at"], "customer_lat": req["customer_lat"], "customer_lng": req["customer_lng"],
        })  
    return {"status": "success", "requests": requests_data, "worker_skill": worker_skill, "service_radius_km": service_radius_km, "count": len(requests_data)}
...
# --- WORKER SELF-SERVICE ENDPOINTS (availability / radius / profile) ---

@app.put("/api/workers/availability")
async def update_worker_availability(data: WorkerAvailabilityUpdate):
    if not data.worker_id:
        raise HTTPException(status_code=400, detail="Worker ID is required.")
    result = supabase.table("workers").update({"is_available": data.is_available}).eq("id", data.worker_id).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Worker record not found. Make sure worker_id is workers.id.")
    verify = supabase.table("workers").select("id, user_id, is_available").eq("id", data.worker_id).single().execute()
    return {"status": "success", "worker_id": verify.data["id"], "user_id": verify.data["user_id"], "is_available": verify.data["is_available"]}

@app.put("/api/workers/radius")
async def update_worker_radius(data: WorkerRadiusUpdate):
    if not data.worker_id:
        raise HTTPException(status_code=400, detail="Worker ID is required.")
    if data.service_radius_km < 1 or data.service_radius_km > 100:
        raise HTTPException(status_code=400, detail="Service radius must be between 1 and 100 km.")
    result = supabase.table("workers").update({"service_radius_km": float(data.service_radius_km)}).eq("id", data.worker_id).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Worker record not found. Make sure worker_id is workers.id.")
    verify = supabase.table("workers").select("id, user_id, service_radius_km").eq("id", data.worker_id).single().execute()
    return {"status": "success", "worker_id": verify.data["id"], "user_id": verify.data["user_id"], "service_radius_km": verify.data["service_radius_km"]}

@app.put("/api/workers/profile")
async def update_worker_profile(data: WorkerProfileUpdate):
    if not data.user_id:
    ...
    user_check = supabase.table("users").select("id").eq("id", data.user_id).maybe_single().execute()
    if not user_check.data:
        raise HTTPException(status_code=404, detail="User record not found. user_id must be users.id.")

    worker_check = supabase.table("workers").select("id, user_id").eq("id", data.worker_id).maybe_single().execute()
    if not worker_check.data:
        raise HTTPException(status_code=404, detail="Worker record not found. worker_id must be workers.id.")
    if str(worker_check.data["user_id"]) != str(data.user_id):
        raise HTTPException(status_code=403, detail="This worker does not belong to this user.")

    user_result = supabase.table("users").update({"name": data.name.strip(), "phone": data.phone.strip()}).eq("id", data.user_id).execute()
    if not user_result.data:
        raise HTTPException(status_code=500, detail="User profile could not be updated.")
    ...
    worker_result = (
        ...
        .update({
            ...
            "service_radius_km": float(data.service_radius_km),
        })
        .eq("id", data.worker_id).eq("user_id", data.user_id).execute()
    )
    if not worker_result.data:
        raise HTTPException(status_code=500, detail="Worker profile could not be updated.")

    final_user = supabase.table("users").select("id, name, phone").eq("id", data.user_id).single().execute()
    final_worker = supabase.table("workers").select("id, user_id, skill_category, hourly_rate, upi_id, service_radius_km, is_available").eq("id", data.worker_id).single().execute()
    return {"status": "success", "message": "Profile updated successfully.", "user": final_user.data, "worker": final_worker.data}