...
def get_worker_requests(worker_id: str, worker_lat: float, worker_lng: float, search_radius_km: float = 15.0):
    """
    Return pending jobs assigned to this worker, enriched with customer/location data.
    Phase 2 uses service_id as the job title/category because the current schema shown
    in main.py does not expose a separate services/jobs table.
    """
    worker_res = supabase.table("workers").select(
        "skill_category, is_available"
    ).eq("id", worker_id).single().execute()

    if not worker_res.data:
        raise HTTPException(status_code=404, detail="Worker not found")

    worker_skill = (worker_res.data.get("skill_category") or "").strip().lower()

    response = supabase.table("bookings").select(
        "id, group_id, customer_id, service_id, status, price, "
        "customer_lat, customer_lng, expires_at, customers(name)"
    ).eq("worker_id", worker_id).eq("status", "pending").execute()
    ...
    for req in response.data:
        ...
        if expires_at:
            try:
                ...
                now_aware = datetime.now(expiry_dt.tzinfo) if expiry_dt.tzinfo else datetime.utcnow()
                if expiry_dt < now_aware:
                    supabase.table("bookings").update({"status": "expired"}).eq("id", req["id"]).execute()
                    continue
            except Exception:
                pass

        service_id = str(req.get("service_id") or "Service")
        service_name = service_id

        # If service_id is being stored as the human-readable service/category,
        # enforce the worker's skill category. If it is a UUID/ID, Phase 3 should
        # connect it to the actual service table.
        if worker_skill and service_name.lower() != worker_skill:
            # Also allow the common "<Skill> ..." format.
            if not service_name.lower().startswith(worker_skill):
                continue

        customer_lat = req.get("customer_lat")
        customer_lng = req.get("customer_lng")
        if customer_lat is None or customer_lng is None:
            continue

        dist_km = haversine_distance(
            worker_lat, worker_lng, customer_lat, customer_lng
        )
        ...
        requests_data.append({
            ...
            "eta_mins": calculate_eta_mins(dist_km),
            "expires_at": req.get("expires_at"),
            "status": req.get("status"),
        })

    requests_data.sort(key=lambda item: item["distance_km"])
    return {
        "status": "success",
...
async def update_worker_availability(data: WorkerAvailabilityUpdate):
    """
    Availability toggle used by the worker dashboard.
    This only controls whether new jobs may be offered to the worker.
    """
    res = supabase.table("workers").update({
